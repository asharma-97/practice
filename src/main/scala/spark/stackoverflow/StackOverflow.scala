...
object StackOverflow extends StackOverflow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val lines   = sc.textFile("src/main/resources/datasets/stackoverflow.csv")
...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val answers = postings.filter(_.postingType == 2).map(a => (a.id, a))
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map{ g => (g._2.toList.head._1, answerHighScore(g._2.map(_._2).toArray))}
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(x => (firstLangInTag(x._1.tags, langs).getOrElse(-1), x._2)).filter(_._1 != -1).cache()
  }
